...
<form method="POST" use:enhance>
    ...
    <h1 class="mt-6 mb-2 font-semibold leading-none tracking-tight">Custom Fields</h1>
            ...
            <FormField {form} name={field.id}>
                <FormControl>
                    {#snippet children({ props })}
                        <FormLabel>{field.name}</FormLabel>
                        <Input
                            {...props} bind:value={$formData.customFields[field.id]} type="number" step={field.options.step}
                            placeholder={field.options.placeholder || ""}
                        />
                    {/snippet}
                </FormControl>
